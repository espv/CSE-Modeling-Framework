...
extern "C" {
	void (*lsRun)(void);
	unsigned long next_schedsim_event;
	unsigned long next_netsim_event;
    unsigned long numevs;
}
namespace ns3 {
/* STEIN */
...
DefaultSimulatorImpl::ProcessOneEvent (void)
{
  ...
  /* STEIN */
  // We need to update next_ns3_event
  next_netsim_event = m_events->PeekNext().key.m_ts;

#if 0 // OYSTEDAL
  // Check which goes first, SchedSim or Ns-3
  if(next_schedsim_event <= next_netsim_event)
	  /* lsRun() will update next_schedsim_event, and
             may schedule new Ns-3 events. */
    lsRun();
#endif
...
DefaultSimulatorImpl::Stop (Time const &time)
{
  ...
  //if (time > 10000000000)
    //std::cout << "DefaultSimulatorImpl::Stop, time: " << time << std::endl;
...
{
  // When time is very high, it is most likely a thread running in an "infinite" loop
  /*if (time > 10000000000) {
    std::cout << "DefaultSimulatorImpl::Schedule, time: " << time << ", number of times a high time has been used " << ++cnt << std::endl;
  }*/
...
DefaultSimulatorImpl::ScheduleNow (EventImpl *event)
{
  ...
  //std::cout << "DefaultSimulatorImpl::ScheduleNow, Now: " << Now() << std::endl;
  Scheduler::Event ev;
  ev.impl = event;
